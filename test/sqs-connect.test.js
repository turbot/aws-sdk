/**
 * SQS v3 Connect Integration Tests
 *
 * Tests that connect('SQS') returns a v3 proxy instead of a v2 client.
 */

const { expect } = require("chai");

describe("@turbot/aws-sdk SQS v3 integration", function () {
  // Store original env
  let originalEnv;
  ...
  describe("connect('SQS') returns v3 proxy", function () {
    it("returns an object with v3 proxy methods", function () {
      const taws = require("../index");
      const sqs = taws.connect("SQS", { region: "us-east-1" });

      expect(sqs).to.be.an("object");
      expect(sqs.getQueueUrl).to.be.a("function");
      expect(sqs.getQueueAttributes).to.be.a("function");
      expect(sqs.setQueueAttributes).to.be.a("function");
      expect(sqs.removePermission).to.be.a("function");
      expect(sqs.listQueues).to.be.a("function");
      expect(sqs.sendMessage).to.be.a("function");
      expect(sqs.changeMessageVisibility).to.be.a("function");
    });

    it("has _client property (v3 SQSClient)", function () {
      const taws = require("../index");
      const sqs = taws.connect("SQS", { region: "us-east-1" });

      expect(sqs._client).to.be.an("object");
      // v3 client has send method
      expect(sqs._client.send).to.be.a("function");
    });

    it("has _config property with region", function () {
      const taws = require("../index");
      const sqs = taws.connect("SQS", { region: "ap-southeast-2" });

      expect(sqs._config).to.be.an("object");
      expect(sqs._config.region).to.equal("ap-southeast-2");
    });

    it("is NOT a v2 AWS.SQS instance", function () {
      const taws = require("../index");
      const AWS = require("aws-sdk");
      const sqs = taws.connect("SQS", { region: "us-east-1" });

      // v2 client would be instanceof AWS.SQS
      expect(sqs instanceof AWS.SQS).to.be.false;
    });
  });

  describe("buildV3Config for SQS", function () {
  ...
  describe("SQS proxy with proxy agent via connect()", function () {
    it("passes proxy through when configured via TURBOT_CONFIG_ENV", function () {
      ...
      expect(sqs._config.requestHandler).to.be.an("object");
    });
  });
});
